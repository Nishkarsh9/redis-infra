pipeline {
    ...
    stages {
        ...
        stage('Clone Repository') {
            ...
            steps {
                ...
                sh 'git clone https://github.com/Nishkarsh9/redis-infra.git'
            }
        }
        ...
        stage('Terraform Init') {
            ...
            steps {
                withCredentials([[$class: 'AmazonWebServicesCredentialsBinding', credentialsId: 'aws-credentials']]) {
                    dir('redis-infra/redis-terraform') {
                        sh 'terraform init'
                    }
                }
            }
                ...
                withCredentials([sshUserPrivateKey(credentialsId: 'private-ssh-key', keyFileVariable: 'SSH_PRIVATE_KEY')]) {
                    script {
                        sh '''
                            ...
                            BASTION_PUBLIC_IP=$(ansible-inventory -i redis-infra/redis-role/aws_ec2.yml --list | jq -r '._meta.hostvars[._Bastion_server.hosts[0]].public_ip_address')

                            cd redis-infra/redis-role
                            ...
                            echo "Running Ansible playbook on redis-server-0 via bastion host..."
                            ANSIBLE_HOST_KEY_CHECKING=False ANSIBLE_SSH_ARGS='-o ForwardAgent=yes -o ConnectTimeout=60' ansible-playbook -i aws_ec2.yml redis_playbook.yml --private-key=$SSH_PRIVATE_KEY -u ubuntu
                            cd ../..
                        '''
                    }
                }
    ...
    post {
        ...
        success {
            ...
            mail to: 'nishkarshkumar11@gmail.com', subject: "Jenkins Pipeline Success", body: "Pipeline executed successfully! Build URL: ${BUILD_URL}"
        }
        failure {
            ...
            mail to: 'nishkarshkumar11@gmail.com', subject: "Jenkins Pipeline Failure", body: "Pipeline failed. Build URL: ${BUILD_URL}"
        }
        aborted {
            ...
            mail to: 'nishkarshkumar11@gmail.com', subject: "Jenkins Pipeline Aborted", body: "Pipeline was manually aborted. Build URL: ${BUILD_URL}"
        }
    }
}
